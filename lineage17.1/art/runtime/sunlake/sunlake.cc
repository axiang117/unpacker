...
#include "../libdexfile/dex/class_accessor-inl.h"
...
void Sunlake::invokeAllMethods() {
  ...
  for (const DexFile* dex_file : Unpacker_dex_files_) {
    ...
    for (; class_idx < dex_file->NumClassDefs(); class_idx++) {
      ...
      ObjPtr<mirror::Class> klass = class_linker->ResolveType(dex_file->GetClassDef(class_idx).class_idx_, h_dex_cache, h_class_loader);
      if (klass == nullptr) {
...
void Sunlake::dumpMethod(ArtMethod *method, int nop_size) {
  ...
  uint32_t code_item_size = (uint32_t)method->GetDexFile()->GetCodeItemSize(*code_item);
  ...
  if (nop_size != 0) {
    ...
    memset(buf + code_item_start_size, 0, nop_size);
  }
